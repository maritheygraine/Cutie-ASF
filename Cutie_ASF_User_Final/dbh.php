...
if(mysqli_connect_error()){                                                     //CHECK CONNECTION
    die("Database connection failed!: " . mysqli_connect_error(). "<br>");              //PROMPT ERROR IF CONNECTION FAILED
}
else{
    ...
    if(mysqli_query($connect,'SELECT 1 FROM pet LIMIT 1') == false){        //CHECK IF ACCOUNT TABLE EXISTS
        ...
        if(mysqli_query($connect, $createPetTable)){
            //echo "Pet table creation success<br>";
        }else{
            echo "Pet table creation failed<br>";
        }
    }
    if(mysqli_query($connect,'SELECT 1 FROM transaction LIMIT 1') == false){        //CHECK IF TRANSACTION TABLE EXISTS
        ...
        if(mysqli_query($connect, $createTransactionTable)){
            //echo "transaction table creation success<br>";
        }else{
            echo "transaction table creation failed<Br>";
        }
    }
...
function db_pet_a_pets($admin_id = -1){
    ...
    $has_param = ($admin_id > -1) ? "WHERE a_id=$admin_id" : "WHERE availability='available'" ;
 ...
 *  @param int $admin_id (OPTIONAL) ID of the admin
 * 
 *  @return bool
 */
...
/**
 *  BOOLEAN: Checks if pet type exists. Returns True if pet type exists; returns false otherwise
 * 
 *  @param string $type Type of pet
 *  @param int $admin_id (OPTIONAL) ID of admin
 * 
 *  @return bool
 */
function db_pet_type_exists($type, $admin_id = -1){

    $has_param = ($admin_id > -1) ? "AND a_id=$admin_id" : "" ;
    $sql = "SELECT * FROM pet WHERE type='$type' $has_param";
    $result = mysqli_query($GLOBALS['connect'], $sql);
...
function db_pet_type_search($pet_type, $admin_id = -1){
    
    $has_param = ($admin_id > -1) ? "AND a_id=$admin_id" : "" ;
    $sql = "SELECT * FROM pet WHERE type='$pet_type' $has_param";
    $query = mysqli_query($GLOBALS['connect'], $sql);
    $row_num = 0;
...
/**
 *  BOOLEAN: Returns true if pet's availability was set to 'adopted' successfully; returns false otherwise
 * 
 *  Sets the pet availabilty to 'adopted', meaning that the current request has been denied
 * 
 *  @param int $p_id Pet ID
 *  @param string $pet_name Pet name
 *  @param string $pet_type Pet type
 ...
 *  @return array
 */
function db_view_u_petsReqAdopt($u_id){
    $has_param = ($u_id > -1) ? "AND u_id=$u_id" : "";
    $sql = "SELECT * FROM v_pet WHERE (p_avail='requested' OR p_avail='adopted') $has_param";
    $query = mysqli_query($GLOBALS['connect'], $sql);
    $row_num = 0;
